...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Solve the basketball lineup optimization problem"
    )
    parser.add_argument(
        "--away",
        action="append",
...
def get_active_players(team: Team, away_player_names: list[str]) -> list[str]:
    ...
    return [
        player.name for player in team.players if player.name not in away_player_set
    ]
...
def build_starting_lineup(
    ...
    remaining_players = [
        player_name for player_name in active_players if player_name not in starters
    ]
...
def filter_power_combos(
    active_players: list[str], power_combos: list[list[str]]
) -> list[list[str]]:
    ...
    return [
        ...
        if len(power_combo) >= 2
        and all(player_name in active_player_set for player_name in power_combo)
    ]


def filter_required_final_period_players(
    active_players: list[str], required_player_names: list[str]
) -> list[str]:
...
def format_player_names(
    player_names: list[str], player_index_by_name: dict[str, int]
) -> str:
...
def build_period_start_times(
    periods_per_half: list[int], minutes_per_half: int
) -> list[str]:
...
def add_player_count_constraints(
    model, is_on_court, num_players: int, num_periods: int, on_court_per_period: int
):
...
def add_playtime_balance_constraints(
    model, is_on_court, num_players: int, num_periods: int, periods_per_half: list[int]
):
        ...
        for player_idx_b in range(num_players):
            if player_idx_a < player_idx_b:
                ...
                model.add(
                    ...
                    == total_periods_played[player_idx_a]
                    - total_periods_played[player_idx_b]
                )
    ...
    for player_idx in range(num_players):
        first_half_on = model.new_int_var(
            0, first_half_periods, f"first_half_on_p{player_idx}"
        )
        second_half_on = model.new_int_var(
            0, second_half_periods, f"second_half_on_p{player_idx}"
        )
        model.add(
            first_half_on
            == sum(
                is_on_court[(player_idx, period_idx)]
                for period_idx in range(first_half_periods)
            )
        )
        model.add(
            second_half_on
...
def add_no_consecutive_off_constraints(
    model, is_on_court, num_players: int, num_periods: int
):
...
def add_starting_lineup_constraints(
    model, is_on_court, players: list[str], starting_lineup: list[str]
):
    ...
    for player_idx, player_name in enumerate(players):
        model.add(
            is_on_court[(player_idx, 0)]
            == int(player_name in preferred_starting_player_set)
        )
...
def add_final_period_constraints(
    ...
    required_player_indices = [
        player_indices[player_name] for player_name in required_player_names
    ]
    required_player_vars = [
        is_on_court[(player_idx, num_periods - 1)]
        for player_idx in required_player_indices
    ]
...
def add_power_combo_objective(
    ...
    for combo_index, power_combo in enumerate(power_combos):
        combo_flags = []
        combo_player_indices = [
            player_indices[player_name] for player_name in power_combo
        ]
        for period_idx in range(num_periods):
            combo_on = model.new_bool_var(f"power_combo_{combo_index}_t{period_idx}")
            players_on = sum(
                is_on_court[(player_idx, period_idx)]
                for player_idx in combo_player_indices
            )
    ...
    for period_idx in range(num_periods):
        ...
        combo_flags_for_period = [
            combo_flags[period_idx] for combo_flags in power_combo_together_flags
        ]
        model.add(sum(combo_flags_for_period) >= 1).only_enforce_if(period_power_combo)
        model.add(sum(combo_flags_for_period) == 0).only_enforce_if(
            period_power_combo.Not()
        )
...
def render_solution(
    ...
    if status in (OPTIMAL, FEASIBLE):
        period_start_times = build_period_start_times(
            periods_per_half, minutes_per_half
        )
...
def solve_team_lineup(
    ...
    starting_lineup = build_starting_lineup(
        active_players, requested_start_players, config.on_court_per_period
    )

    if len(config.periods_per_half) != 2:
        raise ValueError(
            "periods_per_half must contain exactly two entries, e.g. [6, 6] or [6, 5]."
        )
    ...
    add_player_count_constraints(
        model, is_on_court, num_players, num_periods, config.on_court_per_period
    )
