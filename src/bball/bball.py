...
def main():
    ...
    for p in range(num_players):
        for t in range(num_periods):
            x[(p, t)] = model.new_bool_var(f"x_p{p}_t{t}")
    ...
    for p in range(num_players):
        total = model.new_int_var(0, num_periods, f"total_p{p}")
    ...
    for p1 in range(num_players):
        for p2 in range(num_players):
            if p1 < p2:
                diff = model.new_int_var(-num_periods, num_periods, f"diff_{p1}_{p2}")
    ...
    for p in range(num_players):
        ...
        model.add_bool_or([x[(p, 0)], x[(p, second_half_start)]])

    # --- Players who start OFF at start must not also be OFF at end ---
    for p in range(num_players):
        # x[p,0] == 0 => x[p, last] == 1
        model.add_bool_or([x[(p, 0)], x[(p, num_periods - 1)]])

    # --- Objective: maximise periods where Mila and Katrina are on together ---
    kat_index = players.index("Katrina")
    together_vars = []
    for t in range(num_periods):
        both_on = model.new_bool_var(f"mila_kat_t{t}")
        # both_on == 1 iff Mila and Katrina both on
        model.add(x[(mila_index, t)] + x[(kat_index, t)] == 2).only_enforce_if(both_on)
        model.add(x[(mila_index, t)] + x[(kat_index, t)] != 2).only_enforce_if(both_on.Not())
    ...
    if status == cp_model.OPTIMAL or status == cp_model.FEASIBLE:
        ...
        for t in range(periods_per_half):
            on_court = []
            off_court = []
            for p, name in enumerate(players):
            ...
            first_table.add_row(str(t + 1), ", ".join(on_court), ", ".join(off_court))
        ...
        for t in range(periods_per_half, num_periods):
            on_court = []
            off_court = []
            for p, name in enumerate(players):
                if solver.Value(x[(p, t)]) == 1:
            ...
            second_table.add_row(str(t + 1), ", ".join(on_court), ", ".join(off_court))
